...
function GroupCard({ g }: { g: Group }) {
  return (
    ...
    >
      <span className="flex h-12 w-12 shrink-0 items-center justify-center rounded-xl bg-brand-50 text-2xl dark:bg-brand-500/20">
        {g.emoji}
      </span>
...
export default function Dashboard() {
  ...
  return (
    <div>
      {/* Tabs */}
      <div className="mb-5 flex gap-1 rounded-xl bg-gray-100 p-1 text-sm font-medium dark:bg-slate-800">
        {(['groups', 'friends'] as Tab[]).map((t) => (
          <button
            key={t}
            onClick={() => setTab(t)}
            className={`flex-1 rounded-lg py-2 capitalize transition ${
              tab === t ? 'bg-white shadow-sm dark:bg-slate-700' : 'text-gray-500 dark:text-slate-400'
            }`}
          >
        ...
        <div>
          <div className="mb-4 flex items-center justify-between">
            <div>
              ...
              <p className="text-sm text-gray-500 dark:text-slate-400">Multi-person shared expenses</p>
            </div>
        ...
        <div>
          <div className="mb-4 flex items-center justify-between">
            <div>
              ...
              <p className="text-sm text-gray-500 dark:text-slate-400">1-on-1 personal expenses</p>
            </div>
            ...
            {personalGroups.map((g) => {
              ...
              return (
                <Link
                  ...
                  className="card flex items-center gap-4 p-4 transition hover:shadow-md"
                >
                  <span className="flex h-12 w-12 shrink-0 items-center justify-center rounded-full bg-brand-50 text-2xl dark:bg-brand-500/20">
                    👤
                  </span>
                  <div className="min-w-0">
                    <div className="truncate font-semibold">{g.name}</div>
